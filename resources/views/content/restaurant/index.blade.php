<x-app-layout>
    ...
    <x-slot name="style">
        <style>
            body{
                background-image: linear-gradient(rgba(0,0,0,0.5),rgba(0,0,0,0.5)), url("/images/restaurant-interior.jpg");
                    ...
                    @auth
                        @if (Auth::user()->hasRole('admin'))
                            <div class="py-2 w-full bg-green-700 hover:bg-green-600 my-2 text-center text-white">
                                <a href="{{ route('restaurant.create') }}">Create Restaurant</a>
                            </div>
                        @endif
                    @endauth
                    @forelse ($restaurants as $restaurant)
                    @php
                        App\Http\Controllers\RestaurantController::autoupdate($restaurant);
                    @endphp
                    <div class="bg-white shadow-md  rounded-3xl p-4 my-2 border-2 border-black">
                        <div class="flex-none lg:flex">
                            <div class=" h-64 w-64 lg:h-48 lg:w-48   lg:mb-0 mb-3">
                            ...
                            <div class="flex-auto ml-3 justify-evenly py-2">
                                <div class="flex flex-wrap ">
                                    <div class="w-full flex-none text-xs text-blue-700 font-medium ">
                                        <b class="{{ $restaurant->status == 'Open' ? 'text-green-500' : 'text-red-500' }}">
                                            {{ $restaurant->status }}
                                        </b>
                                    </div>
                                    <h2 class="flex-auto text-lg font-medium">{{ $restaurant->restaurant_name }}</h2>
                                </div>
                                <p class="mt-3"></p>
                                <div class="flex py-4  text-sm text-gray-600">
                                    <div class="flex-1 inline-flex items-center">
                                        ...
                                        <p class="">{{ $restaurant->restaurant_address }}</p>
                                    </div>
                                    <div class="flex-1 inline-flex items-center">
