        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    <form action="{{ route('airplane.update', $airplane) }}" method="post">
                        @csrf
                        @method('PUT')
                        <div class="form-group my-2">
                            <label for="airplane_name" class="w-full">Airplane Name</label>
                            <input type="text" value="{{ $airplane->airplane_name }}" name="airplane_name" id="airplane_name" class="w-full rounded">
                        </div>
                        <div class="form-group my-2">
                            <label for="airplane_description" class="w-full">Airplane Description</label>
                            <textarea name="airplane_description" id="airplane_description" class="w-full" rows="10">{{ $airplane->airplane_description }}</textarea>
                        </div>
                        <div class="form-group my-2">
